<?php
    ...
    $more_string =
        preg_match('/1\.(5\.[2-9]|[6-9]\.[0-9])/', Grocery_CRUD::VERSION)
            ? $this->l('list_more') : "Más";

?>

<?php
    $CI =& get_instance();
    //$idperfil = $CI->session->userdata('idperfil');
    $idperfil = '1'; //$CI->ion_auth->get_users_groups($CI->session->userdata('user_id'))->row()->id;
?>
    ...
    <tr>
        ...
        <td <?php if ($unset_delete) { ?> style="border-left: none;"<?php } ?>>
                <div class="only-desktops"  style="white-space: nowrap">
                    <?php
                        $activa_boton = '';
                        if(($idperfil==3) && !($row->idestado == 1 || $row->idestado == 2 || $row->idestado == 3 || $row->idestado == 5)){
                            $activa_boton = 'boton_inactivo';
                        }
                    ?>
                    <?php if(!$unset_edit){?>
                        <a class="btn btn-default <?php echo $activa_boton; ?>" href="<?php echo $row->edit_url?>"><i class="fa fa-pencil"></i> <?php echo $this->l('list_edit'); ?></a>
                    <?php } ?>
                        ...
                        <?php if ($show_more_button) { ?>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <?php if (!$unset_delete) { ?>
                                        <li>
                                            <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row <?php echo $activa_boton; ?>">
                            ...
                            <?php } else {
                                ...
                                if (!$unset_delete) { ?>
                                    <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row btn btn-default <?php echo $activa_boton; ?>">
                                        <i class="fa fa-trash-o text-danger"></i>
                                        <span class="text-danger"><?php echo $this->l('list_delete')?></span>
                                    </a>
                                <?php } ?>
        ...
        <?php foreach($columns as $column){?>
            <td <?php echo stristr($row->{$column->field_name},'-$') ? 'style="color:red;"' : '' ?> <?php echo stristr($row->{$column->field_name},'$') ? 'align="right" nowrap' : '' ?>>
